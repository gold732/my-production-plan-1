...
# 2. AI 컨설턴트 로직 (Gemini 2.5-Flash-Lite 반영 및 로테이션 강화)
def get_ai_consultant(prompt, context_summary):
    ...
    for key in available_keys:
        try:
            ...
            system_instruction = f"생산관리 전문가로서 다음 데이터를 분석하세요: {context_summary}"
...
# 4. 최적화 엔진 (임금 반영 버그 수정을 위해 모든 변수를 인자로 받음) [cite: 130-160]
def solve_production_plan(D, domain, reg, ot, h, l, inv, back, mat, sub, stime, wdays, ot_lim, w0, i0, ifinal):
    ...
    # 모든 비용 변수를 인자값으로 직접 계산 [cite: 141-144]
...
with tab1:
    if st.button("🚀 최적 생산계획 수립 실행"):
        # 프리징 및 데이터 꼬임 방지: 세션 초기화
        st.session_state['success'] = False 
        with st.spinner('파라미터 반영 중...'):
            try:
                # 모든 사이드바 변수를 인자로 명시적 전달 (반영 버그 해결)
...
with tab3:
    ...
    if prompt := st.chat_input("계획에 대해 물어보세요."):
        ...
        sumry = f"비용:{m.cost() if st.session_state['success'] else 0}, 모드:{opt_mode}"
