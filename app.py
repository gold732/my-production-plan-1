...
with st.sidebar:
    ...
    st.markdown("---")
    st.subheader("🏭 외주 전략")
    enable_outsourcing = st.toggle("외주(Outsourcing) 허용", value=True, help="체크 해제 시 하청 생산을 하지 않고 자체 생산으로만 대응합니다.")
        ...
        with st.spinner('최적화 수행 중...'):
            try:
                ...
                if sol.solver.termination_condition == TerminationCondition.optimal:
                    ...
                    st.error("❌ 최적해를 찾지 못했습니다. 외주를 허용하거나 인당 근무 시간을 늘려보세요.")
            except Exception as e:
...
with tab3:
    ...
    if prompt := st.chat_input("가동률 리스크나 비용 최적화에 대해 물어보세요."):
        st.session_state.messages.append({"role": "user", "content": prompt})
        with st.chat_message("user"): st.markdown(prompt)
        
        # [수정 3] AI에게 전달하는 데이터 문맥(Context)에 가동률 정보를 명시적으로 포함
        if st.session_state['success']:
            m = st.session_state['res']
            u_list = st.session_state['utils']
            u_str = ", ".join([f"{i+1}월 가동률:{val:.1f}%" for i, val in enumerate(u_list)])
            ctx = f"총비용:{m.cost():,.0f}, 외주허용여부:{enable_outsourcing}, 상세가동률:[{u_str}], 기말재고:{m.I[len(demand)]()}"
        else:
