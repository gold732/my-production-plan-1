...
def get_ai_consultant(prompt, context_summary):
    ...
    for key in available_keys:
        try:
            ...
            sys_msg = f"원예장비 제조 APP 전문가로서 다음 데이터를 분석하세요. 특히 외주 하청은 부품 단위 조달임을 인지하세요: {context_summary}"
            response = model.generate_content(sys_msg + "\n\n질문: " + prompt)
            return response.text
        except Exception: continue 
    return "❌ AI 연결 실패"

# 3. 사이드바 제어판 (모든 파라미터 15개 유지)
with st.sidebar:
    st.header("🎮 시스템 제어판")
    ...
    st.markdown("---")
    st.subheader("📦 공급망 전략 (강의록 기반)")
    allow_sub = st.toggle("외주 부품 하청(Subcontracting) 허용", value=True)
    ...
    v_c_sub = st.number_input("외주 하청 부품비 (개당)", value=30)
...
def solve_plan(D, domain, reg, ot, h, l, inv, back, mat, sub, stime, wdays, ot_lim, w0, i0, ifinal, can_sub):
    ...
    for t in T:
        ...
        if not can_sub: m.c.add(m.C[t] == 0)
...
with tab1:
    if st.button("🚀 최적 생산계획 수립 실행"):
        ...
        with st.spinner('최적화 계산 중...'):
            try:
                ...
                else: st.error("❌ 최적해 없음. 제약 조건을 확인하세요.")
    ...
    if st.session_state.get('success'):
        ...
        # 메인 그래프 대시보드 (복구 완료)
        st.subheader("📈 월별 생산/공급 흐름 진단")
        fig = go.Figure()
        fig.add_trace(go.Bar(x=list(range(1,7)), y=[m.P[t]() for t in range(1,7)], name="자체 생산", marker_color='royalblue'))
        fig.add_trace(go.Bar(x=list(range(1,7)), y=[m.C[t]() for t in range(1,7)], name="외주 부품 조달", marker_color='lightslategray'))
...
with tab2:
    if st.session_state.get('success'):
        ...
        if sum(u) == 0: st.warning("⚠️ 외주 부품 조달이 경제적이라 판단되어 공장 가동률이 0%입니다.")
        elif max(u) > 100: st.error(f"🚨 과부하 경고: 최대 가동률 {max(u):.1f}% 발생")
...
with tab3:
    ...
    if prompt := st.chat_input("질문하세요."):
        ...
        u_str = ", ".join([f"{i+1}월:{val:.1f}%" for i, val in enumerate(st.session_state.get('utils', []))])
        ctx = f"총비용:{m.cost():,.0f}, 가동률:[{u_str}], 외주허용:{allow_sub}"
