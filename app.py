...
with st.sidebar:
    ...
    v_c_mat = st.number_input("기본 재료비 (개당)", value=10)
    v_c_sub = st.number_input("외주 추가 프리미엄 (개당)", value=20) # 기존 v_c_sub(30)에서 mat(10)을 뺀 차액 예시
...
def solve_production_plan(D, domain, reg, ot, h, l, inv, back, mat, sub, stime, wdays, ot_lim, w0, i0, ifinal):
    ...
    # 수정: 자체 생산분은 mat 적용, 외주 생산분은 (mat + sub) 적용
    m.cost = Objective(expr=sum(
        reg*m.W[t] + ot*m.O[t] + h*m.H[t] + l*m.L[t] + 
        inv*m.I[t] + back*m.S[t] + 
        mat*m.P[t] +            # 내부 생산 재료비
        (mat + sub)*m.C[t]      # 외주 생산비 (재료비 + 외주 프리미엄)
