...
with st.sidebar:
    ...
    v_c_sub = st.number_input("외주 하청 프리미엄 (개당)", value=30) # 용어 직관적 변경
...
def solve_production_plan(D, domain, reg, ot, h, l, inv, back, mat, sub, stime, wdays, ot_lim, w0, i0, ifinal):
    ...
    # [핵심 수정] 외주 비용 = (재료비 + 외주 프리미엄) * 외주량
    # 기존: mat*m.P[t] + sub*m.C[t] 
    # 수정: mat*m.P[t] + (mat + sub)*m.C[t]
    m.cost = Objective(expr=sum(reg*m.W[t] + ot*m.O[t] + h*m.H[t] + l*m.L[t] + 
                                inv*m.I[t] + back*m.S[t] + mat*m.P[t] + (mat + sub)*m.C[t] for t in T), sense=minimize)
...
with tab1:
    ...
    if st.session_state.get('success'):
        ...
        with col_l:
            ...
            # 외주 비용 계산 시 프리미엄 반영
            sub_total = sum((v_c_mat + v_c_sub)*m.C[t]() for t in range(1,len(demand)+1))
            costs = {
                "노무비": sum(v_c_reg*m.W[t]() for t in range(1,len(demand)+1)), 
                "재고비": sum(v_c_inv*m.I[t]() for t in range(1,len(demand)+1)), 
                "자체재료비": sum(v_c_mat*m.P[t]() for t in range(1,len(demand)+1)), 
                "외주총비용": sub_total,
                "기타": m.cost() - (sum(v_c_reg*m.W[t]() for t in range(1,len(demand)+1)) + sum(v_c_inv*m.I[t]() for t in range(1,len(demand)+1)) + sum(v_c_mat*m.P[t]() for t in range(1,len(demand)+1)) + sub_total)
            }
...
with tab3: # AI 상담방 (기존 유지)
    ...
    if prompt := st.chat_input("계획에 대해 질문하세요."):
        ...
        if st.session_state['success']:
            ...
            ctx = f"총비용:{m.cost():,.0f}, 알고리즘:{opt_mode}, 월별가동률:[{u_str}], 기말재고:{m.I[len(demand)]()}"
        else: ctx = "아직 최적화 계획이 수립되지 않았습니다."
