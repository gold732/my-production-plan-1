...
def get_ai_consultant(prompt, context_summary):
    ...
    for key in available_keys:
        try:
            ...
            system_instruction = f"당신은 스마트제조 전문 컨설턴트입니다. 데이터를 분석하세요: {context_summary}"
            response = model.generate_content(system_instruction + "\n\n사용자 질문: " + prompt)
            return response.text
        except Exception: continue 
    return "❌ AI 연결 오류"

# 3. 사이드바: 파라미터 세트 (사용자 요청에 따라 구성 유지)
with st.sidebar:
    st.header("🎮 시스템 제어판")
    ...
    v_c_sub = st.number_input("외주 하청 비용 (추가 프리미엄)", value=30) 
...
def solve_production_plan(D, domain, reg, ot, h, l, inv, back, mat, sub, stime, wdays, ot_lim, w0, i0, ifinal):
    ...
    # [span_3](start_span)[수정 핵심] 외주비(C_t) 계산 시 '재료비 + 외주 프리미엄' 합산 적용[span_3](end_span)
    # 목적함수 Z = ... + (재료비 * 생산량) + ((재료비 + 외주 프리미엄) * 외주량)
...
with tab1:
    ...
    if st.session_state.get('success'):
        ...
        fig.add_trace(go.Scatter(x=list(range(1,len(demand)+1)), y=demand, name="예상 수요", line=dict(color='red', dash='dash')))
        fig.update_layout(barmode='stack')
        st.plotly_chart(fig, use_container_width=True)

# 탭 2, 3 및 세부 UI는 기존과 동일하게 작동합니다.
